 ...
 */
package com.devexperts.tools.tdp;
...
public class ThreadDumpParser {
    ...
    // todo: parameters (make private + accessors)
    ...
    double threshold = 5;
    String state;
    String output = "profile.txt";

    // todo: split into parsing/writing
    public void process(String[] files) {
        if (state != null) {
            for (int i = 0; i < THREAD_STATES.length; i++) {
                ThreadState threadState = THREAD_STATES[i];
                if (threadState.abbreviation.equalsIgnoreCase(state)) {
                    onlyThreadState = i;
                    break;
                }
            }
            if (onlyThreadState < 0)
                throw new InvalidFormatException("State abbreviation is not found: " + state);
        }
        try {
                ...
                try (PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(file + "~"), 100000))) {
                    ...
                    while (line != null) {
                        ...
                        while (indent < line.length() && (line.charAt(indent) == ' ' || line.charAt(indent) == '|'))
                            indent++;
                        if (indent <= (top ? topDepth : methodDepth) * 2)
                            out.println(line);
                        line = in.readLine();
                    }
                }
    ...
    private static class MethodStats {
        ...
        IndexedSet<String, MethodStats> getChildren() {
            if (children == null)
                children = new IndexedSet<>(METHOD_STATS_BY_NAME_INDEXER);
            return children;
        }
    ...
    private static final Indexer<String, MethodStats> METHOD_STATS_BY_NAME_INDEXER = new Indexer<String, MethodStats>() {
        @Override
        public String getObjectKey(MethodStats value) {
            return value.name;
        }
    };
    ...
    private final IndexedSet<String, MethodStats> forwardStats = new IndexedSet<>(METHOD_STATS_BY_NAME_INDEXER);
    private final IndexedSet<String, MethodStats> reverseStats = new IndexedSet<>(METHOD_STATS_BY_NAME_INDEXER);
    ...
    private void parse(BufferedReader in) throws IOException {
        while (line != null) {
            ...
            while (line != null) { // Loop for all threads.
                ...
                if (methods.get(0).equals("sun.misc.Unsafe.park") && !locks) {
                    int i = 1;
                              ...
                              MethodStats outer)
    {
        ...
        for (MethodStats ms : method.children)
            if (ms.counts[threadState] != 0) {
                ...
                if (ms.contributes(threadState, outer, threshold))
    ...
    private void dumpMethodDepth(PrintWriter out, String indent, MethodStats method, MethodStats outer) {
        if ((indent.length() >> 1) > methodDepth)
            return;
        if (method.children == null)
            return;
        ...
        for (int i = 0; i < sorted.length; i++) {
            if (sorted[i].contributes(outer, threshold))
                sorted[n++] = sorted[i];
            else
                skipped++;
        }
